...
!CNe - added following variable
        INTEGER,SAVE,POINTER  ::IDRNQMAX
      TYPE GWFDRNTYPE
...
C3------READ AUXILIARY VARIABLES AND CBC ALLOCATION OPTION.
   ...
   13    FORMAT(1X,'LISTS OF DRAIN CELLS WILL NOT BE PRINTED')
         IPRDRN = 0
         GO TO 10
      ELSE IF(LINE(ISTART:ISTOP).EQ.'QMAX') THEN !CNe - added statement to read QMAX keyword
         WRITE(IOUT,998)
  998    FORMAT(1X,'MAXIMUM DRAIN DISCHARGE ACTIVE')
         IDRNQMAX=1
         DRNAUX(NAUX+1)='QMAX'
         GO TO 10
      END IF
...
C4------ALLOCATE SPACE FOR DRAIN ARRAYs.
      ...
      MXDRN=MXACTD+MXPD
C
!CNe - allocate DRAI; added one value to DRAI dimensions to account for QMAX
      IF(IDRNQMAX.GT.0) THEN
        ALLOCATE (DRAI(NDRNVL+1,MXDRN))
      ELSE
...
C5------READ NAMED PARAMETERS.
 ...
 1000 FORMAT(1X,//1X,I5,' Drain parameters')
     ...
     &                   NUMINST)
          NLST=LSTSUM-LSTBEG
C
!CNe - read QMAX value for parameter K
          IF(IDRNQMAX.GT.0) THEN
              IF(IFREFM.EQ.0) THEN
                 READ(IN,'(F10.0)') QMAX
              ELSE
                READ(IN,*) QMAX
              END IF
            WRITE(IOUT,1001) QMAX
 1001 FORMAT(1X,'MAXIMUM DRAIN DISCHARGE: ', G15.7)
          ENDIF
C
          IF(NUMINST.EQ.0) THEN
C5A-----READ PARAMETER WITHOUT INSTANCES
C
!CNe - because DRAI has different dimensions when QMAX is active, NDRNVL is incremented by one to keep correct dimensions
! IAL is incremented by one to keep the correct number of records to read
            IF(IDRNQMAX.GT.0) THEN
                CALL ULSTRD(NLST,DRAI,LSTBEG,NDRNVL+1,MXDRN,1+1,IN,IOUT,
     &      'DRAIN NO.  LAYER   ROW   COL     DRAIN EL.  STRESS FACTOR',
     &        DRNAUX,20,NAUX,IFREFM,NCOL,NROW,NLAY,5,5,IPRDRN)
!CNe - set last dimension in DRAI array to QMAX
            DRAI(NDRNVL,LSTBEG:(LSTBEG+NLST-1))=QMAX
C
            ELSE
...
! IAL is incremented by one to keep the correct number of records to read
            IF(IDRNQMAX.GT.0) THEN
            CALL ULSTRD(NINLST,DRAI,LSTBEG,NDRNVL+1,MXDRN,1+1,IN,IOUT,
     &      'DRAIN NO.  LAYER   ROW   COL     DRAIN EL.  STRESS FACTOR',
     &        DRNAUX,20,NAUX,IFREFM,NCOL,NROW,NLAY,5,5,IPRDRN)
!CNe - set last dimension in DRAI array to QMAX
            DRAI(NDRNVL,LSTBEG:(LSTBEG+NINLST-1))=QMAX
C
            ELSE
...
C3------IF THERE ARE NEW NON-PARAMETER DRAINS, READ THEM.
   ...
   99       FORMAT(1X,/1X,'THE NUMBER OF ACTIVE DRAINS (',I6,
     1                     ') IS GREATER THAN MXACTD(',I6,')')
            CALL USTOP(' ')
         END IF
!CNe- added +1 to the NAUX value to account for QMAX; added 1 to NDRNVL to keep DRAI dimensions constant in ULSTRD;
        IF(IDRNQMAX.GT.0) THEN
         CALL ULSTRD(NNPDRN,DRAI,1,NDRNVL+1,MXDRN,1,IN,IOUT,
     1     'DRAIN NO.  LAYER   ROW   COL     DRAIN EL.  CONDUCTANCE',
     2     DRNAUX,20,NAUX+1,IFREFM,NCOL,NROW,NLAY,5,5,IPRDRN)
!CNe - set QMAX values to slot NDRNVL+1 because NDRNVL-th slot is used for saving flow
           DRAI(NDRNVL+1,:)=DRAI(NDRNVL,:)
C
        ELSE
...
C1C-----IF THERE ARE ACTIVE DRN PARAMETERS, READ THEM AND SUBSTITUTE
      ...
      IF(NP.GT.0) THEN
         NREAD=NDRNVL-1
         DO 30 N=1,NP
!CNe - substitute parameters when QMAX is true; added +1 to NREAD & NDRNVL & NAUX;
          IF(IDRNQMAX.GT.0) THEN
           CALL UPARLSTSUB(IN,'DRN',IOUTU,'DRN',DRAI,NDRNVL+1,MXDRN,
     1                NREAD+1,MXACTD,NDRAIN,5,5,
...
C6-------IF THE CELL IS EXTERNAL SKIP IT.
      ...
      IF ( EL.LT.BOT ) THEN
        ...
        CALL USTOP(' ')
      END IF
  103 FORMAT('DRAIN SET TO BELOW CELL BOTTOM. MODEL STOPPING. ',
...
C
      DOUBLE PRECISION EEL,QQ,QMAX !CNe - added QQ & QMAX
C     ------------------------------------------------------------------
      CALL SGWF2DRN7PNT(IGRID)
C
...
C7------HEAD IS HIGHER THAN DRAIN. ADD TERMS TO RHS AND HCOF.
      C=DRAI(5,L)
!CNe - if QQ is higher than QMAX, set to QMAX and add to RHS
! where QQ=C*(EL-HHNEW)and add to RHS
...
C
        QQ=C*EL-C*HNEW(IC,IR,IL)
        IF(ABS(QQ).GT.ABS(QMAX)) THEN
            RHS(IC,IR,IL)=RHS(IC,IR,IL)-(-ABS(QMAX))
            GO TO 100
        ENDIF
C
      ENDIF
...
C     ------------------------------------------------------------------
     ...
     1                      VBVL,VBNM
      USE GWFDRNMODULE,ONLY:NDRAIN,IDRNCB,DRAI,NDRNVL,DRNAUX,IDRNQMAX !CNe - added IDRNQMAX
C
      CHARACTER*16 TEXT
      DOUBLE PRECISION HHNEW,EEL,CCDRN,CEL,RATOUT,QQ,QMAX !CNe - added QMAX
C
      DATA TEXT /'          DRAINS'/
C     ------------------------------------------------------------------
      CALL SGWF2DRN7PNT(IGRID)
...
C
        ...
        DEALLOCATE(DRNAUX)
        DEALLOCATE(DRAI)
!CNe - added following statements
        DEALLOCATE(IDRNQMAX)
C
      RETURN
      END
...
!CNe - added following variable
        IDRNQMAX=>GWFDRNDAT(IGRID)%IDRNQMAX
C
      RETURN
      END
...
!CNe - added following variable
        GWFDRNDAT(IGRID)%IDRNQMAX=>IDRNQMAX
C
      RETURN
      END
