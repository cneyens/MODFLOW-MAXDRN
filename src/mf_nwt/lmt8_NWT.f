...
C--COLLECT AND SAVE ALL RELEVANT FLOW MODEL INFORMATION
     ...
     &   CALL LMT8LPF7(ILMTFMT,ISSMT3D,IUMT3D,KKSTP,KKPER,IGRID)
        IF(IUNIT(62).GT.0)
     &   CALL LMT8UPW1(ILMTFMT,ISSMT3D,IUMT3D,KKSTP,KKPER,IGRID)
        IF(IUNIT(37).GT.0)
...
C--RETURN
 1000 CONTINUE
      ...
      END
C
      SUBROUTINE LMT8UPW1(ILMTFMT,ISSMT3D,IUMT3D,KSTP,KPER,IGRID)
C *********************************************************************
C SAVE FLOW ACROSS THREE CELL INTERFACES (QXX, QYY, QZZ), FLOW RATE TO
C OR FROM TRANSIENT FLUID-STORAGE (QSTO), AND LOCATIONS AND FLOW RATES
C OF CONSTANT-HEAD CELLS FOR USE BY MT3D.  THIS SUBROUTINE IS CALLED
...
C
      ...
      USE GWFDRNMODULE,ONLY:NDRAIN,DRAI,IDRNQMAX,NDRNVL !CNe - added IDRNQMAX & NDRNVL
      CHARACTER*16 TEXT
      DOUBLE PRECISION HHNEW,EEL,CCDRN,CEL,QQ,QMAX !CNe - added QMAX
C
C--SET POINTERS FOR THE CURRENT GRID
...
C--OTHERWISE, CALCULATE Q=C*(EL-HHNEW).
          IF(HHNEW.GT.EEL) THEN
            QQ=CEL - CCDRN*HHNEW
C
!CNe - if QMAX is active, top off QQ
! QMAX is the NDRNVL-th+1 element of DRAI
            IF(IDRNQMAX.GT.0) THEN
                QMAX=DRAI(NDRNVL+1,L)
!CNe - skip if QMAX is negative
                IF(QMAX.LT.0.) GO TO 101
C
                IF(ABS(QQ).GT.ABS(QMAX)) QQ=-ABS(QMAX)
            ENDIF
C
 101        CONTINUE
         ENDIF
...
C DATE CREATED: 7-25-2013
     ...
     &                       LKFLOWTYPE,NLKFLWTYP
      USE LMTMODULE,    ONLY:ILAKFLOWS,NLAKCON
C
      IMPLICIT NONE
C
      CHARACTER*16 TEXT
